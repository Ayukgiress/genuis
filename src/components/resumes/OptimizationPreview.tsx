  ...
  return (
    <div className="space-y-6">
        ...
        <div className="p-6 rounded-2xl bg-zinc-900/50 border border-red-500/20">
          ...
          <ul className="space-y-3">
            ...
            {(!result.weaknesses || result.weaknesses.length === 0) && (
              <li className="text-sm text-zinc-500 italic">No significant weaknesses found.</li>
            )}
          </ul>
        </div>
        ...
        <div className="p-6 rounded-2xl bg-zinc-900/50 border border-green-500/20">
          ...
          <ul className="space-y-3">
            ...
            {(!result.suggestions || result.suggestions.length === 0) && (
              <li className="text-sm text-zinc-500 italic">Generate suggestions to see optimizations.</li>
            )}
          </ul>
        </div>
      </div>
      ...
      <div className="flex justify-end">
        <button className="px-6 py-3 bg-primary text-black font-bold rounded-xl hover:opacity-90 transition-all flex items-center gap-2">
          <svg viewBox="0 0 24 24" className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth="2">
